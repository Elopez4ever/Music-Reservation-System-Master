          ...
          <el-col :span="8">
            ...
            <el-card class="card-item" shadow="hover">
              ...
              <div class="room-stats">
                ...
                <div class="room-stat-item">
                  <div class="room-stat-header">
                    <span class="room-type"><i class="el-icon-monitor"></i> 多媒体琴房</span>
                    <span class="room-value">{{ roomStats.multimedia || 45 }}% 空闲</span>
                  </div>
                  <el-progress :percentage="roomStats.multimedia || 45" :color="'#E6A23C'" :stroke-width="15"></el-progress>
                </div>
                
                <div class="room-stat-item">
                  <div class="room-stat-header">
                    <span class="room-type"><i class="el-icon-cpu"></i> 实验室</span>
                    <span class="room-value">{{ roomStats.lab || 60 }}% 空闲</span>
                  </div>
                  <el-progress :percentage="roomStats.lab || 60" :color="'#409EFF'" :stroke-width="15"></el-progress>
                </div>
              </div>
            </el-card>
            ...
            <el-card class="card-item" shadow="hover">
              <div slot="header" class="card-header">
                <span><i class="el-icon-star-on"></i> 今日热门琴房</span>
              </div>
              ...
              <div v-else>
                <el-collapse v-model="activeNames" accordion>
                  <el-collapse-item
                    ...
                    :key="room.roomId"
                    :title="(index + 1) + '. ' + room.roomName"
                    :name="room.roomId"
                  >
                    <div class="hot-room-content">
                      <div class="hot-room-info">
                        <span>
                          <i class="el-icon-office-building"></i>
                          {{ room.buildingName }}
                        </span>
                        <el-tag size="small" type="danger">预约次数: {{ room.reservationCount }}</el-tag>
                      </div>
                      <el-button type="primary" size="mini" plain class="hot-room-btn" @click="handleQuickReserveRoom(room.roomId, room.roomName)">
                        <i class="el-icon-notebook-2"></i> 立即预约
                      </el-button>
                    </div>
                  </el-collapse-item>
                </el-collapse>
                <div v-if="!hotRooms || hotRooms.length === 0" class="empty-data">
...
export default {
  ...
  methods: {
    ...
    fetchTodayHotRooms() {
      this.loading = true;
      roomApi.getTodayHotRooms().then(res => {
        ...
        console.error('获取热门琴房失败:', error);
        this.hotRooms = [];
        this.loading = false;
        this.$message.error('获取热门琴房数据失败');
      });
    },
    // 获取琴房类型列表
    fetchRoomTypeList() {
    ...
    // 快速预约指定琴房
    handleQuickReserveRoom(roomId, roomName) {
      // 检查用户是否登录
      const token = localStorage.getItem('token');
      if (!token) {
        this.$confirm('您尚未登录，是否前往登录页面?', '提示', {
